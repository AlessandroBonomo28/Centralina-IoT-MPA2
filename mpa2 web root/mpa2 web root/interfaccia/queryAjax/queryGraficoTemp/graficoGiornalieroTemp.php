...
$query = "
			 select  avg(temperatura) as temperatura, hour(dataOra) as ora from Letture 
			 where year(dataOra) like '".$anno."' and
			 month(dataOra) like '".$mese."' and
			 day(dataOra) like '".$giorno."' and
			 temperatura is not null and
...
else if(mysqli_num_rows($risultato) > 0)
{
	...
	{
		$istanza = array( "ora" => $riga['ora'] ,"temperatura" => $riga['temperatura'] );
